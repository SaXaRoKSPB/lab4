...
for items in all_things:
    sum_value += all_things[items][1]
ans = 15
max_place = 8
things_place = [all_things[item][0] for item in all_things]
things_value = [all_things[item][1] for item in all_things]
sum_place = len(things_value)
table = [[0 for i1 in range(max_place + 1)] for i2 in range(sum_place + 1)]
    ...
    for column in range (max_place+1):
        if line == 0 or column == 0:                
            ...
            if things_place[line-1] <= column:
                table[line][column] = max(things_value[line-1] + table[line-1][column-things_place[line-1]], table[line-1][column])
            else:
    ...
    for key, things_value in all_things.items():
        if things_value[0] == search[0] and things_value[1] == search[1]:
            if not(key in selected_stuff):
                ...
                break     
ans -= sum_value
ans_table = [['' for i in range (2)] for j in range (4)]
line = 0
column = 0
for item in selected_stuff:
    sum_place_item = all_things[item][0]
